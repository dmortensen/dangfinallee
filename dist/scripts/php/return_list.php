<?php
  ...
  if ( isset($_REQUEST['key']) ) {
    ...
    $reply_list = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/dist/data/replied.txt');
      ...
      for ( $i = 0; $i < count($guest_array); $i++ ) {
        foreach ( $guest_array[$i] as $guest ){
          if ( trim(strtolower($key)) == trim(strtolower($guest))) {
            $pattern = '/'.$key.'/i';
            preg_match( $pattern, $reply_list, $matches);

            if($matches !== []) {
              echo json_encode(array('status' => 'replied', 'result' => array()));
              exit ();
            }
